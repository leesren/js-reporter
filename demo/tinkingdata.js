(function(root, factory) {
	...
	try {
			...
			var tdCommon = {
					...
					_date: {
							_format: function(d) {
									...
									return d.getFullYear() + "-" + pad(d.getMonth() + 1) + "-" + pad(d.getDate()) + " " + pad(d.getHours()) + ":" + pad(d.getMinutes()) + ":" + pad(d.getSeconds()) + "." + padMilliseconds(d.getMilliseconds())
							}
					},
					_code: {
							...
							_base64Encode: function(data) {
									...
									} while (i < data.length);enc = tmp_arr.join("");
			...
			var tdStore = {
					...
					_save: function() {
							tdClient._cookie._set(tdClient._cookie._getCookieName(), JSON.stringify(this._state), 73e3, td.param.crossSubDomain)
					},
					_init: function() {
							...
							_set: function(name, value, days, crossSubDomain, isSecure) {
									...
									if (days !== 0) {
											...
											if (String(days).slice(-1) === "s") {
													date.setTime(date.getTime() + Number(String(days).slice(0, -1)) * 1e3)
											} else {
			...
			var tdEvent = {
					...
					_send: function(p, callback) {
							...
							if (p.type == "user_set" || p.type == "user_setOnce" || p.type == "user_add" || p.type == "user_del") {} else {
					...
					if (dom && tdCommon._check._isObject(dom)) {
							...
							for (var type in dom) {
									...
									if (link && tdCommon._check._isArray(link)) {
											...
											case "id":
													for (var i = 0; i < link.length; i++) {
															var element = document.getElementById(link[i]);
															if (element != null) {
